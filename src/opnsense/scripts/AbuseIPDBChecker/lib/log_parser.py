...
Handles parsing OPNsense firewall logs and extracting external IPs
"""
...
class FirewallLogParser:
    """Handles all firewall log parsing operations"""
    ...
    def list_external_to_internal_connections(self):
        """List ALL external→internal connections with detailed info"""
        if not os.path.exists(self.config['log_file']):
    ...
    def _parse_connection_details(self, line):
        """Parse detailed connection information from log line"""
        try:
            if '] ' not in line:
                return None
    ...
    def _debug_parse_line(self, line):
        """Parse line for debugging purposes with error handling"""
        try:
            if '] ' in line:
                csv_part = line.split('] ', 1)[1]
                fields = csv_part.split(',')
